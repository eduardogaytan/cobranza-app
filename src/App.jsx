...
function esCarteraVencida(cl) {
  ...
  if (abono <= 0) return false;
  if (semana >= plazo && cobro > 0) return true;
  if (cobro > abono) return true;
  return false;
}
...
function AdminPanel({ asesor, onLogout }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const cobros = await api(
        `cobros?enviado=eq.true&aprobado=eq.${tab === "aprobados" ? "true" : "false"}&select=*,cliente:clientes(id,nombre,codigo,cobro_semana,abono_original,pago_con_intereses,num_semana,poblado:poblados(nombre,ruta:rutas(id,nombre))),asesor:asesores(id,nombre)`
      );
      ...
      try {
        ...
        try {
          ...
          for (const r of rows) {
            ...
            const clientesRuta = todosClientes.filter(c => pobladoIds.includes(c.poblado_id));
          ...
          console.error('[RutasScreen] Error cargando stats:', e);
        }
      } catch (e) { console.error(e); }
      setLoading(false);
    };
